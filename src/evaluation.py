...
def rouge_wrapper(prediction, ground_truth, metric="r"):
    try:
        result = rouge.get_scores(prediction, ground_truth, avg=True)
        return result["rouge-1"][metric], result["rouge-2"][metric], result["rouge-l"][metric]
    except:
        return 0.0, 0.0, 0.0


def f1_score(prediction, ground_truths, normalize_fn: Callable[[str], str] = lambda x: x):
    return max([f1(prediction, gt, normalize_fn) for gt in ground_truths] + [f1(prediction, ', '.join(ground_truths), normalize_fn)])
...
def rouge_score(prediction, ground_truths, metric="f"):
    ...
    if (
        ...
        return 0.0, 0.0, 0.0
    scores = [rouge_wrapper(prediction, gt, metric) for gt in ground_truths]
